...
public class AttendanceListSimulator extends Simulator {
  public AttendanceListSimulator(Player player, List<Player> players) {
  ...
  public void execute() {
    ...
    while (true) {
      ...
      for (Player player : this.players) {
        ...
        // set GAMMA
        int GAMMA = player.getGroup().size() * Config.SMALL_DELTA;
        ...
        for (Event event : events) {
          if (event.getType() == Event.Type.SEND) { // event type is send
            if (event.getCommand().equals("NEW_GROUP")) {
              ...
              if (player.getGroup().size() > 0 && player.getId() == player.getGroup().getPlayers().last().getId()) {
                Event receiveListEvent = new Event(Event.Type.RECEIVE, "LIST", event.getTime());
                addEvent(t + Config.SMALL_DELTA, next(player), receiveListEvent);
              }
...
//                System.out.print(player.getL() + GAMMA);
//                System.out.print(',');
...
//                System.out.println();
            ...
            if (event.getCommand().equals("NEW_GROUP") && t <= event.getTime()) {
              ...
              if (t <= event.getTime() + GAMMA) {
                ...
                if (player.getId() != player.getGroup().getPlayers().last().getId()) {
                  Event receiveListEvent = new Event(Event.Type.RECEIVE, "LIST", event.getTime(), player);
                  addEvent(t + Config.SMALL_DELTA, next(player), receiveListEvent);
                }
              }
            }
